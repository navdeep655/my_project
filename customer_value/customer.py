...
with st.expander("📊 Click to view Graphs"):
    ...
    graph_option = st.sidebar.selectbox(
        ...
        (
            "CLV Distribution",
            "CLV vs Total Spending",
            "Customer Segments",
            "Correlation Heatmap",
            "CLV vs Age by Segment"
        )
    )

    st.markdown(f"### 🔍 {graph_option}")
    st.markdown("Get deeper insight into customer behavior and value drivers.")

    if graph_option == "CLV Distribution":
        st.image("CLV Distribution Plot.png", caption="Distribution of Customer Lifetime Value")
        st.info("This graph shows the distribution of CLV among customers.")

    elif graph_option == "CLV vs Total Spending":
        st.image("purchase_freq_vs_clv.png", caption="CLV vs Total Spending")
        st.info("How total spending influences predicted CLV.")

    elif graph_option == "Customer Segments":
        st.image("CLV segments.png", caption="CLV Segmentation")
        st.info("Customers segmented based on predicted CLV.")

    elif graph_option == "Correlation Heatmap":
        st.image("heatmap.png", caption="Correlation Heatmap")
        st.info("See which features influence CLV the most.")

    elif graph_option == "CLV vs Age by Segment":
        st.image("Age_vs_clv.png", caption="CLV vs Age by Segment")
        st.info("How customer age relates to CLV across segments.")
